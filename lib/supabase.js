...
export function calculateYuanPrice(coinsPrice) {
  return coinsPrice / 100;
}

export async function buyProductWithCash(userId, productId, quantity = 1) {
  try {
    const { data: product, error: productError } = await supabase
      .from('products')
      .select('id, price_in_coins, price_in_yuan, farmer_revenue_ratio, platform_revenue_ratio, name')
      .eq('id', productId)
      .single();
    ...
    const cashAmount = (product.price_in_yuan || calculateYuanPrice(product.price_in_coins)) * quantity;
    const farmerRevenueRatio = product.farmer_revenue_ratio || 0.70;
    const platformRevenueRatio = product.platform_revenue_ratio || 0.30;
    
    const farmerRevenue = cashAmount * farmerRevenueRatio;
    const platformRevenue = cashAmount * platformRevenueRatio;
    ...
    const { data: order, error: orderError } = await supabase
      ...
      .insert({
        ...
        product_id: productId,
        quantity: quantity,
        total_price: Math.round(product.price_in_coins * quantity),
    ...
    const { data: item, error: itemError } = await supabase
      ...
      .upsert({
        ...
        product_id: productId,
        quantity: quantity,
        acquired_type: 'purchase_cash',
        acquired_at: new Date().toISOString()
      }, { onConflict: 'user_id, product_id' })
      .select()
      .single();
    
    if (itemError) console.error('Error creating user item:', itemError);
    ...
    console.error('Error buying product with cash:', error);
    return { success: false, error: error.message };
  }
}

export async function createOrderWithCoins(userId, productId, quantity = 1) {
  try {
    const { data: product, error: productError } = await supabase
      .from('products')
      .select('id, price_in_coins, name')
      .eq('id', productId)
      .single();
    ...
    const totalPrice = product.price_in_coins * quantity;
    ...
    if (profileError) throw profileError;
    if (!profile || profile.green_coins < totalPrice) {
    ...
    const { error: updateError } = await supabase
      .from('profiles')
      .update({ 
        green_coins: profile.green_coins - totalPrice,
        updated_at: new Date().toISOString()
      })
      .eq('id', userId);
    ...
    console.error('Error creating order with coins:', error);
    return { success: false, error: error.message };
  }
}

export async function createWithdrawal(userId, amountCoins) {
  try {
    const yuanAmount = amountCoins / 200;
    ...
    if (profileError) throw profileError;
    if (!profile || profile.green_coins < amountCoins) {
      throw new Error('积分不足');
    }
    
    if (yuanAmount < 10) {
      throw new Error('最低提现10元（2000积分）');
    }
    
    const { error: deductError } = await supabase
      .from('profiles')
      .update({ 
        green_coins: profile.green_coins - amountCoins,
        updated_at: new Date().toISOString()
      })
      .eq('id', userId);
    ...
    return { success: true, withdrawal, yuanAmount };
  } catch (error) {
